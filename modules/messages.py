...
Техподдержка: @sup_novgorod_yabloko_bot
''',

    'not_found_address' : 
'''<b>Ошибка</b>: Адрес не был найден
Убедитесь, что ввели полное название улицы и правильно ли.
...
''',
    ...
    'im_vote' : '''
Отправьте фотографии ваших бюллетеней для их сохранения
''',

    'im_vote_need_reg' : '''
Отправьте фотографии ваших бюллетеней для их сохранения

Вы не зарегистрированны! Отправить фотографии бюллетеней сможете только после регистрации!
''',
...
''',
    ...
    'send_vote' : '''Отправьте фотографию вашего бюллетеня за <u>{}</u>''',

    'already_get_photo' : 'Фото принято!',

    'we_have_photo' : '''Ваше фото бюллетеня за <u>{}</u>

<b>Если хотите поменять фото, то удалите это и загрузите другое</b>''',
